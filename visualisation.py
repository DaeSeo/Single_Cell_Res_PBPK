...
import argparse
import os
from matplotlib.ticker import ScalarFormatter # Added for plain number formatting

def plot_single_tissue(tissue_df, tissue_name, target_name, save_path=None):
    """
    Generates three separate subplots for one tissue:
    1. Upper: Receptor Occupancy (%) over time
    2. Middle: Bound Drug Concentration (nM) over time
    3. Lower: Serum Concentration (nM) over time (Log Scale with Plain Numbers)
    """
    # Professional plot settings
    sns.set_theme(style="whitegrid")
    ...
    # 2. Middle Plot: Bound Drug (nM)
    sns.lineplot(data=tissue_df, x='time', y='bound_nM', hue='cell_type', 
                 ax=ax2, palette='tab10', linewidth=2.5, legend=False)
    ax2.set_ylabel("Bound Drug (nM)", fontsize=12, fontweight='bold')
    ax2.set_ylim(bottom=0)

    # 3. Bottom Plot: Serum Concentration (nM)
    # Applying Log Scale for better TMDD visualization
    sns.lineplot(data=tissue_df, x='time', y='plasma_conc', color='black', 
                 ax=ax3, linewidth=2, linestyle='--')
    
    # Set Y-axis to Log Scale
    ax3.set_yscale('log')
    
    # Format Y-axis to show plain numbers instead of scientific notation (10^n)
    ax3.yaxis.set_major_formatter(ScalarFormatter())
    ax3.yaxis.get_major_formatter().set_scientific(False)
    ax3.yaxis.get_major_formatter().set_useOffset(False)
    
    ax3.set_ylabel("Serum Conc (nM)", fontsize=12, fontweight='bold')
    ax3.set_xlabel("Time (days)", fontsize=12)
    
    # Adjust lower limit for log scale visibility
...
def main():
    parser = argparse.ArgumentParser(description="PBPK Result Visualizer (3-Subplot Mode)")
    parser.add_argument('--Target', type=str, default='EGFR', help="Target receptor (e.g., EGFR)")
    parser.add_argument('--Tissue', type=str, required=True, help="Tissue name or 'all'")
    parser.add_argument('--Drug', type=str, default='Cetuximab', help="Drug name")
    parser.add_argument('--Dose', type=float, default=250.0, help="Dose in mg/m2")
    ...
    if args.Tissue.lower() == 'all':
        for tissue in tissues_present:
            tissue_df = df[df['tissue'] == tissue]
            filename = f"plot_{args.Target.lower()}_{args.Drug.lower()}_{tissue.lower()}_{args.Dose}mg.png"
            plot_single_tissue(tissue_df, tissue, args.Target, save_path=os.path.join(output_dir, filename))
    else:
        target_tissue = args.Tissue.lower()
        tissue_df = df[df['tissue_lower'] == target_tissue]
        if not tissue_df.empty:
            filename = f"plot_{args.Target.lower()}_{args.Drug.lower()}_{target_tissue}_{args.Dose}mg.png"
            plot_single_tissue(tissue_df, args.Tissue, args.Target, save_path=os.path.join(output_dir, filename))
